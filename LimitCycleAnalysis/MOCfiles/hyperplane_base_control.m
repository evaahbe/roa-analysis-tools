function [eta,vispline,vidotspline] = hyperplane_base_control(sys,fns)
    ...
    uloc    = zeros(sys.udim,length(tvec));
    
    %classical v-vectors
    for k = 1:length(tvec)
        xloc(:,k) = ppval(sys.xorb,tvec(k));
        uloc(:,k) = ppval(sys.uorb,tvec(k));
        fx(:,k) = fns.system_dyn(xloc(:,k),uloc(:,k),sys);
    ...
    if sys.trafo == 1
        ...
        if sys.xdim == 2
            ...
            eta_one = median(vclass,2)/norm(median(vclass,2));
        ...
        if isfield(fns,'system_dyn_deriv')  % --> with dynamics derivative, if available 
            for k = 1:length(tvec)
                dfxdt = fns.system_dyn_deriv(xloc(:,k),uloc(:,k),sys,tvec(k));
                vidot(:,k) = dfxdt./sqrt(sum(fx(:,k).^2)) + fx(:,k)./(-(sum(fx(:,k).^2))^(3/2)) .* dot(fx(:,k),dfxdt);
            end
        else                            % --> numerically, otherwise
            for k = 1:length(tvec)
                xa = ppval(sys.xorb,tvec(k)+dt);
                ua = ppval(sys.uorb,tvec(k)+dt);
                xb = ppval(sys.xorb,tvec(k)-dt); 
                ub = ppval(sys.uorb,tvec(k)-dt);
                fxa = fns.system_dyn(xa,ua,sys);
                fxb = fns.system_dyn(xb,ub,sys);
