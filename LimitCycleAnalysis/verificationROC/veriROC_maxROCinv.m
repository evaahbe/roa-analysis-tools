function veriROC_maxROCinv(sys,fns,filenames,pinit,numsets,~)
    ...
    while jj<=numsets.iteration_max
        ...
        if step == 1   
            ...
            [multis, multi_mats] = find_multiplier(y,rho,fns,onHP,sys,numsets,degs,Qvals,alpha);
            step = 2;
            save(filenames.rocintermedresults,'-append','step','multis','multi_mats')
        elseif step == 2 
            fprintf(1,'- step: Metric optimization\n\n')
            [alpha,Qvals] = optimize_Metric(y,rho,fns,onHP,sys, numsets,degs,alpha_list,multis,multi_mats);
            alpha_list = [alpha_list;alpha];
            step = 1;
            jj = jj+1;
            save(filenames.rocintermedresults,'-append','alpha_list','Qvals','step','alpha')
        end
            
    end
    
    save(filenames.rocfinalresults, 'multis','multi_mats','alpha_list','Qvals','alpha','Kvals','gamma','sys','numsets')   
end



function [multis,multi_mats] = find_multiplier(y,rho,fns,onHP,sys,numsets,degs,Qvals,alpha)
    ...
    for i = 1:sys.Nhp-1
        ...
        for j= 1:sys.uncer_count
            ...
            eq1a = y' * (- C_cond + neg_fac - sm1 * (1 - rho'* M *rho) ) * y;
            eq1b = y' * sm1 * y;
            ...
            Vdot = clean(rho'*(delMdeltau * taudotnum + Mrhodot)*rho + 2*rho'*M*rhodot,numsets.clean_thresh);
            
            eq4a = - Vdot + neg_fac + h1 * (1 - rho'* M *rho)-1e-4;
        ...
        eq2 = taudotden  - s2 * (1 - rho'* M *rho);  
        eq3 = -((alphai - rho'*Smatfix*rho) * s3 + (rho'* M *rho - 1));
...
function [alphares,Qvals] = optimize_Metric(y,rho,fns,onHP,sys, numsets,degs,alpha_list,multis,multi_mats)
    ...
    for i=1:sys.Nhp-1
        ...
        for j = 1:sys.uncer_count
            ...
            eq1 = y' * (- C_cond + neg_fac - sm1fix * (1 - rho'* M *rho) ) * y;
            ...
            Vdot = clean(rho'*(delMdeltau * taudotnum + Mrhodot)*rho + 2*rho'*M*rhodot,numsets.clean_thresh);
            eq4 = - Vdot + neg_fac + h1fix * (1 - rho'* M *rho)-1e-4;
        ...
        eq2 = taudotden  - s2fix * (1 - rho'* M *rho);  
        eq3 = -((alphatot - rho'*Smatfix*rho) * s3fix + (rho'* M *rho - 1));
        
        F = [F, sos(eq2), sos(eq3)];
    end

    [sol] = solvesos(F, ObjFun, numsets.sdpsetting2,[coeflist(:);alphaarray(:)]);
