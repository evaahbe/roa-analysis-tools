function [sys,fns,pinit] = initializeGoluskin(sys,fns,pinit,ana_type)


    load LC_Goluskin.mat
    ...
    sys.mu = 3; %3 for nominal Goluskin
    ...
    fns.system_dyn= str2func('dynamics_gol');
    fns.system_dyn_deriv = str2func('deriv_dynamics_gol');
    fns.system_linearization = str2func('linear_dynamics_gol');
    fns.system_dyn_poly = str2func('dynamics_gol'); % dynamics for VDP already polynomial
    
    if strcmp(ana_type,'ROC')
        % for ROC computations provide the uncertain dynamics
        fns.system_dyn_poly_uncertain = str2func('uncertain_dynamics_gol');
        sys.uncermax = [0.25*sys.mu]; %column vector of maximum variation of each uncertain parameter. If this is to be maximized just fill zeros or a desired minimum size
